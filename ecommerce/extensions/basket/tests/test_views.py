...
class BasketSummaryViewTests(CourseCatalogTestMixin, CourseCatalogMockMixin, LmsApiMockMixin, ApiMockMixin, TestCase):
    ...
    def assert_quantity_field(self, product, show_quantity_field):
        """Assert whether a quantity field should be shown for the passed product.

        Args:
            product (Product): The product that is added to the basket.
            show_quantity_field (bool): Whether or not the quantity field should be shown.
        """
        self.create_basket_and_add_product(product)
        response = self.client.get(self.get_full_url(self.path))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content['show_quantity_field'], show_quantity_field)
    ...
    def assert_basket_switch_items(self, original_item, exchange_item):
        """Assert that the original basket item has been exchanged by the exchange one.
        This happens when a basket contains a seat (quantity: 1) and the quantity is changed to > 1,
        in which case the seat is exchanged for an enrollment codes. Also if the original one
        is an enrollment code and quantity is 1 then the product is exchanged for a seat product.
        If this confuses you, I share your pain.
