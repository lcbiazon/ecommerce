...
class CouponViewSetFunctionalTest(CouponMixin, CourseCatalogTestMixin, CourseCatalogMockMixin, ThrottlingMixin,
    ...
    def test_update_max_uses_field(self):
        ...
        self.client.post(COUPONS_LINK, json.dumps(self.data), 'application/json')
        coupon = Product.objects.get(title=self.data['title'])
        details_response = self.client.get(reverse('api:v2:coupons-detail', args=[coupon.id]))
        detail = json.loads(details_response.content)
        self.assertEqual(detail['max_uses'], max_uses)

        updated_max_uses = 5
        self.data.update({'max_uses': updated_max_uses})
        path = reverse('api:v2:coupons-detail', args=[coupon.id])
        response = self.get_response_json('PUT', path, self.data)
        self.assertEqual(response['max_uses'], updated_max_uses)
    ...
    def test_creating_coupon_with_max_uses_and_single_use_set(self):
    ...
    @ddt.data(6, '6')
    def test_creating_coupon_with_max_uses_and_multi_use_set(self, max_uses):
    ...
    @ddt.data(-3, 0, '', 'string')
    def test_creating_coupon_with_invalid_max_uses(self, max_uses):
    ...
    @ddt.data(-3, 0, '', 'string')
    def test_updating_coupon_with_invalid_max_uses(self, max_uses):
        ...
        details_response = self.client.get(reverse('api:v2:coupons-detail', args=[coupon.id]))
        detail = json.loads(details_response.content)
        self.assertEqual(detail['max_uses'], 2)

        self.data.update({'max_uses': max_uses})
        path = reverse('api:v2:coupons-detail', args=[coupon.id])
        response = self.client.put(path, json.dumps(self.data), 'application/json')
